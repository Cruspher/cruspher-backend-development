...
export class NewsService {
  constructor(
    ...
    @InjectRepository(NewsContent)
    private news_content_repository: Repository<NewsContent>,
    @InjectRepository(NewsLanguage)
    private news_language_repository: Repository<NewsLanguage>,
    @InjectRepository(NewsEmotions)
    private news_emotions_repository: Repository<NewsEmotions>,
  ...
  async addNews(dto: addNewsDto, image?: any) {
    ...
    const languagesData = await this.news_language_repository
  ...
  async editNews(dto: editNewsDto, image?: any) {
    const news = await this.news_repository.findOne({
      where: { id: dto.newsId },
    });

    if (!news) {
      throw new HttpException(
        'news з вказаним айді не існує',
    ...
    //Remove old content
    const oldNewsContent = await this.news_content_repository.find({
      where: { news: news },
    });
  ...
  async getOneNews(dto) {
    const defaultLanguage = await this.news_language_repository.findOne({
      where: { isDefault: true },
    });
    if (!defaultLanguage) {
      throw new HttpException(
        'language з вказаним айді не існує',
        HttpStatus.NOT_FOUND,
      );
    }
    const additionalLanguage = !!dto.userLanguage
      ? dto.userLanguage
      : defaultLanguage.id;

    const news = await this.news_repository
  ...
  async removeNews(newsId: number) {
    ...
    //Remove old emotion
    const oldNewsEmotion = await this.news_emotions_repository.find({
      where: { news: news },
    });
