...
export class AuthService {
  ...
  async registerUser(dto: RegisterUserDto) {
    ...
    const admins = ['support@cruspher.com'];
  ...
  async createDefaultUserElements(userId) {
    ...
    const shopElementsList = await this.shopElement_repository
      .createQueryBuilder('ShopElement')
      .where('ShopElement.id IN (:...ids)', {
        ids: process.env.DEFAULT_ELEMENTS_IDS.split(','),
      })
      .getMany();
  ...
  async login(dto) {
    ...
    //     'Ваш аккаунт не активований, підвердіть, будь ласка, активацію акаунту у вашій поштовій скриньці',
    //   );
    // }
