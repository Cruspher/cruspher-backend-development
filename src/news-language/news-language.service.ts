...
export class NewsLanguageService {
  constructor(
    ...
    public newsLanguage_repository: Repository<NewsLanguage>,
  ) {}

  async addLanguage(dto: addLanguageDto) {
    const isHaveLanguage = await this.newsLanguage_repository.count({
      where: { name: dto.name },
    });
    if (!!isHaveLanguage) {
      throw new HttpException(
        'This language already used',
  ...
  async setDefaultLanguage(dto) {
    ...
    const savedLanguages = await this.newsLanguage_repository.save(
      newLanguages,
    );
  ...
  async editNewsLanguage(dto) {
    const language = await this.newsLanguage_repository.findOne({
